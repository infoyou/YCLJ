...
- (void)transLoginData:(NSString *)userName passWord:(NSString *)passWord
{
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@/leju/signin/", YCLJ_HOST_URL];
...
- (void)saveHouseData
{
    ...
    HouseModel *houseModel = [HouseModel newWithDict:houseDict];
...
- (void)transHouseData
{
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@/leju/plan/create/", YCLJ_HOST_URL];
    [ZTHttpTool post:urlStr
              params:paramDict
             success:^(id json) {
...
//                         _houseName = resultDict[@"name"];
                         _houseNo = resultDict[@"no"];
                         
                         // 更改本地数据
                         NSString *modifySql = [NSString stringWithFormat:@"UPDATE Solution SET houseNo = '%@' where houseId = '%@'", _houseNo, _houseId];
                         [HouseFmdbTool modifyData:modifySql];

                         // 上传数据文件到服务端
                         [self uploadFileMehtod:_zipPath no:_houseNo];
                     } else {
...
- (void)uploadFileMehtod:(NSString *)filePath no:(NSString *)no
{
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@/leju/plan/upload_file/", YCLJ_HOST_URL];
